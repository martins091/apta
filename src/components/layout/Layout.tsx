...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Close mobile menu and cleanup when location changes
  useEffect(() => {
    if (mobileOpen) {
      setMobileOpen(false);
  ...
  // Handle mobile submenu item click
  const handleMobileSubItemClick = (href: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setMobileOpen(false);
    setMobileExpanded(null);
    setTimeout(() => {
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground font-sans">
      ...
      >
        <div className="container mx-auto px-6 flex items-center justify-between">
          ...
          {/* Mobile Menu Button */}
          <button
            className="lg:hidden text-gray-700 p-2 hover:text-[#E8B84B] z-50 relative"
        ...
        {mobileOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 bottom-0 w-full max-w-sm bg-white z-50 flex flex-col overflow-y-auto shadow-2xl"
            >
              <div className="pt-24 pb-12 px-6 flex flex-col">
                {NAV.map((item) => (
                  <div key={item.label} className="border-b border-gray-100">
                    {item.items ? (
                            ...
                            >
                              <div className="pb-3 pl-3 space-y-1">
                                {item.items.map((sub) => (
                                  <div
                                    key={sub.label}
                                    onClick={(e) => handleMobileSubItemClick(sub.href, e)}
                                    className="block py-2.5 px-3 text-sm text-gray-600 hover:text-[#5C1A2E] hover:bg-[#5C1A2E]/4 rounded-sm transition-colors cursor-pointer"
                                  >
                                    {sub.label}
                                  </div>
                                ))}
                                <div
                                  ...
                                  className="block py-2.5 px-3 text-xs font-mono tracking-widest uppercase text-[#1A7A8A] hover:text-[#5C1A2E] cursor-pointer"
                                >
                                  View all →
                                </div>
                              </div>
                      ...
                      <div
                        onClick={() => handleMobileNavClick(item.href)}
                        className="block py-4 text-lg font-serif text-gray-800 hover:text-[#5C1A2E] transition-colors cursor-pointer"
                      >
                        {item.label}
                      </div>
                    )}
                ...
                <div className="mt-8 space-y-3">
                  <Button 
                    asChild 
                    className="w-full bg-[#5C1A2E] text-white hover:bg-[#7A2240] rounded-none py-5 text-base font-bold h-auto"
                    onClick={() => setMobileOpen(false)}
                  >
